...
- [Quick Event server](#quick-event-server)
...
- [Процесс авторизации:](#процесс-авторизации)
  ...
  - [API-функции авторизации с помощью email:](#api-функции-авторизации-с-помощью-email)
    - [Локальная регистрация:](#локальная-регистрация)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl)
      - [Ответ:](#ответ)
        - [200 OK:](#200-ok)
        - [400 Bad Request (для примера, не включим email в запрос):](#400-bad-request-для-примера-не-включим-email-в-запрос)
    - [Локальная авторизация:](#локальная-авторизация)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-1)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-1)
      - [Ответ:](#ответ-1)
        - [200 OK:](#200-ok-1)
        - [401 Bad Request (для примера, пароль не верный):](#401-bad-request-для-примера-пароль-не-верный)
  - [API-функции авторизации через ВКонтакте:](#api-функции-авторизации-через-вконтакте)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-2)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-2)
      - [Ответ:](#ответ-2)
        - [200 OK:](#200-ok-2)
        - [400 Bad Request (для примера, токен неверного формата):](#400-bad-request-для-примера-токен-неверного-формата)
- [API-функции подсистемы пользователей:](#api-функции-подсистемы-пользователей)
  - [Административный уровень доступа:](#административный-уровень-доступа)
    - [Создание пользователя:](#создание-пользователя)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-3)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-3)
      - [Ответ:](#ответ-3)
        - [200 OK:](#200-ok-3)
        - [400 Bad Request (для примера, не включим email в запрос):](#400-bad-request-для-примера-не-включим-email-в-запрос-1)
    - [Получение списка пользователей:](#получение-списка-пользователей)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-4)
      - [Ответ:](#ответ-4)
        - [200 OK:](#200-ok-4)
    - [Удаление пользователя:](#удаление-пользователя)
      - [Параметры:](#параметры)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-5)
      - [Ответ:](#ответ-5)
        - [200 OK:](#200-ok-5)
  ...
  - [Пользовательский уровень доступа:](#пользовательский-уровень-доступа)
    - [Получение информации о профиле:](#получение-информации-о-профиле)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-6)
      - [Ответ:](#ответ-6)
        - [200 OK:](#200-ok-6)
  - [Публичный уровень доступа:](#публичный-уровень-доступа)
    - [Получение информации о пользователе:](#получение-информации-о-пользователе)
      ...
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-7)
      - [Ответ:](#ответ-7)
        - [200 OK:](#200-ok-7)
...
- [API-функции подсистемы локаций](#api-функции-подсистемы-локаций)
  - [Административный уровень доступа:](#административный-уровень-доступа-1)
    - [Создание локации:](#создание-локации)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-4)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-8)
      - [Ответ:](#ответ-8)
        - [200 OK:](#200-ok-8)
        - [400 Bad Request (для примера, попытка создать два города с одним наименованием):](#400-bad-request-для-примера-попытка-создать-два-города-с-одним-наименованием)
    - [Обновление локации:](#обновление-локации)
      - [Параметры:](#параметры-2)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-5)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-9)
      - [Ответ:](#ответ-9)
        - [200 OK:](#200-ok-9)
        - [400 Bad Request (для примера, попытка ввода неверной широты):](#400-bad-request-для-примера-попытка-ввода-неверной-широты)
    - [Удаление локации:](#удаление-локации)
      - [Параметры:](#параметры-3)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-10)
      - [Ответ:](#ответ-10)
        - [200 OK:](#200-ok-10)
        - [404 Not Found (для примера, попытка ввода несуществующего uuid):](#404-not-found-для-примера-попытка-ввода-несуществующего-uuid)
  - [Публичный уровень доступа:](#публичный-уровень-доступа-1)
    - [Получение списка локаций:](#получение-списка-локаций)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-11)
      - [Ответ:](#ответ-11)
        - [200 OK:](#200-ok-11)
    - [Получение информации о локации:](#получение-информации-о-локации)
      - [Параметры:](#параметры-4)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-12)
      - [Ответ:](#ответ-12)
        - [200 OK:](#200-ok-12)
- [API-функции подсистемы типов мероприятий](#api-функции-подсистемы-типов-мероприятий)
  - [Административный уровень доступа:](#административный-уровень-доступа-2)
    - [Создание типа мероприятия:](#создание-типа-мероприятия)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-6)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-13)
      - [Ответ:](#ответ-13)
        - [200 OK:](#200-ok-13)
        - [400 Bad Request (для примера, попытка создать два одинаковых типа мероприятия):](#400-bad-request-для-примера-попытка-создать-два-одинаковых-типа-мероприятия)
    - [Удаление типа мероприятия:](#удаление-типа-мероприятия)
      - [Параметры:](#параметры-5)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-14)
      - [Ответ:](#ответ-14)
        - [200 OK:](#200-ok-14)
        - [404 Not Found (для примера, попытка ввода несуществующего uuid):](#404-not-found-для-примера-попытка-ввода-несуществующего-uuid-1)
  - [Публичный уровень доступа:](#публичный-уровень-доступа-2)
    - [Получение списка типом мероприятий:](#получение-списка-типом-мероприятий)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-15)
      - [Ответ:](#ответ-15)
        - [200 OK:](#200-ok-15)
- [API-функции подсистемы новостей](#api-функции-подсистемы-новостей)
  - [Административный уровень доступа:](#административный-уровень-доступа-3)
    - [Создание новости:](#создание-новости)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-7)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-16)
      - [Ответ:](#ответ-16)
        - [200 OK:](#200-ok-16)
        - [400 Bad Request (для примера, попытка ввода слишком длинного текста):](#400-bad-request-для-примера-попытка-ввода-слишком-длинного-текста)
    - [Обновление новости:](#обновление-новости)
      - [Параметры:](#параметры-6)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-8)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-17)
      - [Ответ:](#ответ-17)
        - [200 OK:](#200-ok-17)
        - [400 Bad Request (для примера, попытка ввода неверной широты):](#400-bad-request-для-примера-попытка-ввода-неверной-широты-1)
    - [Удаление новости:](#удаление-новости)
      - [Параметры:](#параметры-7)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-18)
      - [Ответ:](#ответ-18)
        - [200 OK:](#200-ok-18)
        - [404 Not Found (для примера, попытка ввода несуществующего uuid):](#404-not-found-для-примера-попытка-ввода-несуществующего-uuid-2)
...
- [API-функции подсистемы мероприятий](#api-функции-подсистемы-мероприятий)
  - [Административный уровень доступа:](#административный-уровень-доступа-4)
    - [Создание мероприятия:](#создание-мероприятия)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-9)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-23)
      - [Ответ:](#ответ-23)
        - [200 OK:](#200-ok-23)
        - [400 Bad Request (для примера не укажем никакой тип адреса в расписании):](#400-bad-request-для-примера-не-укажем-никакой-тип-адреса-в-расписании)
    - [Редактирование мероприятия](#редактирование-мероприятия)
      - [Параметры:](#параметры-11)
        - [Должен быть со следующими полями:](#должен-быть-со-следующими-полями-10)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-24)
      - [Ответ:](#ответ-24)
        - [200 OK:](#200-ok-24)
    - [Удаление мероприятия:](#удаление-мероприятия)
      - [Параметры:](#параметры-12)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-25)
      - [Ответ:](#ответ-25)
        - [200 OK:](#200-ok-25)
    ...
    - [Назначение ответственного организатора:](#назначение-ответственного-организатора)
      ...
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-26)
      - [Ответ:](#ответ-26)
        - [200 OK:](#200-ok-26)
        - [409 Conflict (Если организатор уже установлен ответственным):](#409-conflict-если-организатор-уже-установлен-ответственным)
    - [Снятие ответственного организатора:](#снятие-ответственного-организатора)
      - [Параметры:](#параметры-14)
      - [Запрос с помощью curl:](#запрос-с-помощью-curl-27)
      - [Ответ:](#ответ-27)
        - [200 OK:](#200-ok-27)
        - [409 Conflict (Если организатор не является ответственным):](#409-conflict-если-организатор-не-является-ответственным)
